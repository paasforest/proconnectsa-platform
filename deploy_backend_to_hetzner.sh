...
scp /home/immigrant/work_platform/manage.py root@${HETZNER_IP}:${HETZNER_DIR}/
scp /home/immigrant/work_platform/requirements.txt root@${HETZNER_IP}:${HETZNER_DIR}/
...
echo ""
echo "🧪 Step 4: Testing public API..."
sleep 5
curl -sS "https://api.proconnectsa.co.za/api/public/providers/?page=1&page_size=1" | \
  python3 -c "import sys, json; d=json.load(sys.stdin); p=(d.get('results') or [{}])[0]; print('✅ Public API ok. Keys:', ','.join(sorted(p.keys()))[:200])"
