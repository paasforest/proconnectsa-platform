#!/usr/bin/env python3
"""
Update Kenneth Nakutepa from client to provider using correct admin credentials
"""
import requests
import json
...
def update_kenneth_to_provider():
    ...
    try:
        ...
        if not kenneth_users:
            ...
            print("\nUsers with 'nakutepa' in name:")
            for user in users:
                if 'nakutepa' in user.get('first_name', '').lower() or 'nakutepa' in user.get('last_name', '').lower():
                    print(f"   - {user.get('first_name')} {user.get('last_name')} ({user.get('email')}) - {user.get('user_type')}")
            return False
