...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const title = `${params.slug.split("-").map(s => s[0]?.toUpperCase() + s.slice(1)).join(" ")} Providers | ProConnectSA`;
  return {
    title,
    description: `Browse verified ${params.slug.replace(/-/g, " ")} providers in South Africa.`,
  };
}
      ...
      <main className="flex-1">
        <section className="py-10 bg-white border-b">
          <div className="container mx-auto px-4">
            <h1 className="text-3xl font-bold text-gray-900 mb-2 capitalize">{params.slug.replace(/-/g, " ")} Providers</h1>
            <p className="text-gray-600">Verified professionals offering {params.slug.replace(/-/g, " ")} services.</p>
            <div className="mt-4">
